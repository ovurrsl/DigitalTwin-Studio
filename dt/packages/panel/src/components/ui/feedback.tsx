...
export function ErrorBox({ children, shield = true }: { children: ReactNode; shield?: boolean }) {
  return (
    ...
    >
      {shield ? (
        <ShieldAlert
          className="mt-px block h-[15px] w-[15px] shrink-0 text-destructive"
          strokeWidth={2.2}
        />
      ) : (
...
export function ScreenTitle({ title, lead }: { title: ReactNode; lead?: ReactNode }) {
  return (
    <>
      ...
      {lead ? (
        <p className="m-0 mt-[2px] text-[12.5px] leading-[1.55] text-muted-fg text-pretty">
          {lead}
        </p>
      ) : null}
    </>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        tone === 'success'
          ? 'border-border bg-popover text-fg'
          : 'border-destructive bg-popover text-destructive',
      )}
